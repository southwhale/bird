 ...
 */
define("todos/todos", [ "bird.class", "bird.action", "bird.request", "bird.validator", "bird.controller", "bird.dom", "bird.array", "bird.event" ], function(require) {
    ...
    (function() {
        ...
        this.initModel = function($, $watcher) {
            ...
            function getItemTpl(count) {
                return '<li><div class="view"><input class="toggle" type="checkbox" name="chkItems" valueVariable="{{checkedItemsValue}}" value="{{count}}"/>' + '<label ondblclick="{{editItem}}">{{title' + count + '}}</label><a class="destroy" onclick="{{removeItem}}"></a>' + '</div><input class="edit" type="text" value="{{title' + count + '}}" onblur="{{showLabel}}"/></li>';
            }
        };
