define(function(require) {
	...
	(function() {
		...
		this.loadImage = function(url, successCallback, errorCallback) {
			...
			if (isFnSuccessCb && img.complete) { // if Image exists in browser cache, call successCallback
				successCallback.call(img);
				return img;
				// return , not need handle 'onload' event
			}

			isFnSuccessCb && (img.onload = function() { //when Image download completed, call successCallback async
				successCallback.call(img);
				//set 'this' pointer of successCallback to Image object
			});
		...
		this.getPageWidth = function() {
			return Math.max(
				(documentElement ? documentElement.scrollWidth : 0), (body ? body.scrollWidth : 0), (viewRoot ? viewRoot.clientWidth : 0),
				0
			);
		};
		...
		this.getPageHeight = function() {
			return Math.max(
				(documentElement ? documentElement.scrollHeight : 0), (body ? body.scrollHeight : 0), (viewRoot ? viewRoot.clientHeight : 0),
				0
			);
		};
