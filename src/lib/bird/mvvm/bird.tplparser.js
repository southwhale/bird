...
define(function(require) {
	...
	(function() {
		...
		var variableAttrs = [
			'class', 'src', 'href', 'value', 'valueVariable', 'style', 'type', 
			'alt', 'for', 'readonly', 'disabled', 'checked', 'selected', 'placeholder', 
			'width', 'height', 'cols', 'border', 'rowspan', 'colspan', 'bgcolor', 'align', 'border',
			'cellpadding', 'cellspacing', 'frame', 'rules', 'summary', 'download', 'coords',
			'media', 'hreflang', 'rel', 'target', 'shape', 'autoplay', 'controls', 'loop', 'muted',
			'preload', 'autofocus', 'form', 'formaction', 'name', 'formmethod', 'formtarget', 'formnovalidate',
			'formenctype', 'cite', 'datetime', 'valuetype', 'open', 'poster'
		];

		var parseFunctionNames = [
			'_parseInlineEvents', '_parseCustomAttr'
		];

		var regExpMap = {
			htmlStartTag: /<([a-zA-Z]+\d*)([^>]*?)\/?>/g, //$1: tagName, $2: propertyStrs
			htmlEndTag: /<\/[a-zA-Z]+\d*>/,
		...
		this._parseInlineEvents = function(str, parsedInfo) {
			var ret = [];
			var inlineEventRE = /\bon([a-z]+)=(['"])(?:[^'"{}]+?\s*)*\s*\{\{([a-z_$][a-z0-9_$().:]*?)\}\}\s*(?:[^'"{}]+?\s*)*\2/ig;
			var arr;
			while (arr = inlineEventRE.exec(str)) {
				ret.push({
					key: arr[1],//key ==> eventType
					variable: arr[3]
				});
			}
		...
		this._parseCustomAttr = function (str, parsedInfo) {
			var ret = [];
			var customAttrRE = /(data-[a-z0-9$]+)=(['"])\s*\{\{([a-z_$][a-z0-9_$]*?)\}\}\s*\2/ig;
			var arr;
			while (arr = customAttrRE.exec(str)) {
				ret.push({
					key: arr[1],//key ==> attributeName
					variable: arr[3]
				});
			}
